 ...
 *//*
    ...
    logic md_pipe_use_encrypt;
    logic md_ret_use_encrypt;
    logic md_head_owner;
    // =====================================================================
    ...
    aes_mode_datapath mode_dp (
        ...
        .internal_output(md_internal_output),
        .pipe_accept_o(md_pipe_accept),
        .pipe_use_encrypt(md_pipe_use_encrypt),
        .ret_use_encrypt(md_ret_use_encrypt),
        .head_owner_o  (md_head_owner)
    );
    ...
    AES_Encrypt_Pipe enc_pipe (
        .clk(clk), .reset(reset),
        .in_valid(md_pipe_in_valid && md_pipe_use_encrypt &&  key_available),
...
always_comb begin
    ...
    gcm_pipe_in_ready = md_pipe_in_ready;
    ...
    if(md_ret_use_encrypt) begin

        // ---------------------------------------------------------
        // GCM INTERNAL RETURN
        // ---------------------------------------------------------

        if(md_head_owner) begin
    ...
    always_comb begin
        bank_busy = 2'b00;
        if (md_pipe_use_encrypt) bank_busy = enc_bank_busy;
...
//  FIXED VERSION
//
//  Key runtime fixes applied:
//    FIX-T1 : Correct GCM return-channel ready handling
//    FIX-T2 : Decouple gcm_pipe_in_ready from decrypt readiness
//    FIX-T3 : Prevent encrypt-pipe output retirement unless consumed
//
// ============================================================================
...
module aes_top (
    ...
    always_comb begin
        ...
        if(md_ret_use_encrypt) begin
            ...
            if(md_head_owner) begin
                ...
                // -----------------------------------------------------
                // FIX-T1 / FIX-T3
                // retire ONLY when GCM side accepts
                // -----------------------------------------------------
    ...
    always_comb begin
        ...
        if(gcm_pipe_override)
            busy = 1'b1;

        if(gcm_pipe_in_valid)
            busy = 1'b1;
